#!/usr/bin/env bash
# Per-turn LLM error/fallback rate for the DeepSeek article runs (s5=aml,
# s6=ref), recovered from each run dir's run.log — PURE LOG PARSING, never
# invokes merging or measure computation.
#
# Usage:
...
uv run python3 ../error_rate_turns.py \
    --layout deepseek --model deepseek-v4-flash \
    "${TURN_ARGS[@]}" \
