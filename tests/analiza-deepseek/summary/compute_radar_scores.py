...
def extract_from_mir(metric: str) -> dict[str, dict[str, float]]:
    ...
    AROM/CoMerger/Boomer/gpt-oss-20b all share one m_i_raport (the original
    scenario_2 run); deepseek-v4-flash's own m_i_raport lives under
    scenario_5's (s5) run dir instead — resolved separately per dataset."""
    out: dict[str, dict[str, float]] = {m: {} for m in METHODS}
    shared_methods = [m for m in METHODS if m != DEEPSEEK_LABEL]
    for ds in DATASETS:
        ...
        if p.exists():
            ...
            for row in csv.DictReader(lines):
                ...
                for method in shared_methods:
                    if row.get("graph") == GRAPH_OF_METHOD[method]:
                        val = row.get("value", "")
        ...
        if p5.exists():
            ...
            for row in csv.DictReader(lines):
                if (row.get("section") == "metrics" and row.get("metric") == metric
                    ...
                    val = row.get("value", "")
                    out[DEEPSEEK_LABEL][ds] = float(val) if val else float("nan")
    return out
