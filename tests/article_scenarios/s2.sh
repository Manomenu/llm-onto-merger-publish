...
#
# Article variant of tests/scenarios/scenario_2.sh: the same single-config
# pipeline (LLM merger + applied-alignments / Boomer / AROM / CoMerger
# baselines + combined report), but batched over a fixed dataset list, and
# --label nests every output under a per-run folder so N independent
# repetitions of the same datasets can coexist.  The AML-input counterpart of
# s3.sh, and the default-backend counterpart of s5.sh.
#
...
# reference-alignment input, i.e. s3.sh.)
#
# Hardcoded config (same as scenario_2.sh):
#   alignment tool aml, max env chars 15000, parallel llm request count 24
#
# Usage:
#   tests/article_scenarios/s2.sh                             # all 3 datasets
#   tests/article_scenarios/s2.sh --label turn1               # outputs under outputs/turn1/s2/
#   tests/article_scenarios/s2.sh --label turn1 --only swo-acm  # single dataset backfill
#   tests/article_scenarios/s2.sh --limit 20000               # override max-env-chars
#   tests/article_scenarios/s2.sh --skip-mine                 # reuse LLM outputs, rerun baselines + report
#   tests/article_scenarios/s2.sh --skip-all                  # reuse ALL caches, only regenerate reports
#
# Flags:
#   --label NAME     Nest outputs under tests/article_scenarios/outputs/NAME/s2/
#                    instead of tests/article_scenarios/outputs/s2/.  NAME is
...
# Same tag scheme as scenario_2.sh (default backend — no model component).
TAG="aml_15k_p${PARALLEL}"
...
echo "========================================"
echo "  s2 / batch run (AML input, default backend)"
...
for spec in "${DATASETS[@]}"; do
  ...
  SCENARIO_DIR="$OUTPUTS_ROOT/s2/$DS_LABEL"
  ...
  echo
  echo "========================================"
  echo "  [s2/${LABEL:+$LABEL/}$DS_LABEL] base: $BASE | candidate: $CANDIDATE"
  ...
  uv run python tests/metrics_and_insights_raport.py \
    ...
    "$OUT_DIR" \
    >"$REPORT_LOG" 2>&1
  echo "  Done [s2/${LABEL:+$LABEL/}$DS_LABEL]. Report: $REPORT_HTML"
done

echo
echo "========================================"
echo "  s2 batch complete."
echo "========================================"
