#!/usr/bin/env bash
# tests/article_scenarios/s3.sh — fixed reference-alignment batch run against
# the default LLM backend (.env Ollama/vLLM, e.g. gpt-oss-20b), labellable for
# repeated-measurement studies.
#
# Article variant of tests/scenarios/scenario_3.sh: feeds the OAEI REFERENCE
...
# Batched over every dataset that has a reference.rdf, and --label nests every
# output under a per-run folder so N independent repetitions can coexist.  The
# reference-input counterpart of s2.sh, and the default-backend counterpart of
# s6.sh.
#
...
#
# Hardcoded config (same as scenario_3.sh):
#   max env chars 15000, parallel llm request count 24
#
# Usage:
#   tests/article_scenarios/s3.sh                             # all 3 datasets
#   tests/article_scenarios/s3.sh --label turn1               # outputs under outputs/turn1/s3/
#   tests/article_scenarios/s3.sh --label turn1 --only swo-acm  # single dataset backfill
#   tests/article_scenarios/s3.sh --limit 20000               # override max-env-chars
#   tests/article_scenarios/s3.sh --skip-mine                 # reuse LLM outputs, rerun baselines + report
#   tests/article_scenarios/s3.sh --skip-all                  # reuse ALL caches, only regenerate reports
#
# Flags:
#   --label NAME     Nest outputs under tests/article_scenarios/outputs/NAME/s3/
#                    instead of tests/article_scenarios/outputs/s3/.  NAME is
#                    also mixed into the merger's --run-nonce, so repeated runs
#                    of the same dataset under different labels share no LLM
#                    prompt prefix (backend-side prompt/prefix caches are
#                    defeated and every run is sampled independently).
...
# Same tag scheme as scenario_3.sh (default backend — no model component).
TAG="ref_15k_p${PARALLEL}"
...
for spec in "${DATASETS[@]}"; do
  ...
  if [ ! -f "$REFERENCE" ]; then
    echo "Error: no reference alignment at $REFERENCE — s3 requires one." >&2
    exit 1
  fi
  ...
  SCENARIO_DIR="$OUTPUTS_ROOT/s3/$DS_LABEL"
  ...
  echo
  echo "========================================"
  echo "  [s3/${LABEL:+$LABEL/}$DS_LABEL] base: $BASE | candidate: $CANDIDATE"
  ...
  uv run python tests/metrics_and_insights_raport.py \
    ...
    "$OUT_DIR" \
    >"$REPORT_LOG" 2>&1
  echo "  Done [s3/${LABEL:+$LABEL/}$DS_LABEL]. Report: $REPORT_HTML"
done

echo
echo "========================================"
echo "  s3 batch complete."
echo "========================================"
